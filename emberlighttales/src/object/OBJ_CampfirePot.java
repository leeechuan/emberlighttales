...
public class OBJ_CampfirePot extends Entity{
	
	GamePanel gp;
    private BufferedImage[] animatedFrames = new BufferedImage[5];
    private int animatedCounter = 0;
	public static final String objName = "Campfire Pot";
	
	public OBJ_CampfirePot(GamePanel gp) {
		...
		for (int i = 0; i < animatedFrames.length; i++) {
	        animatedFrames[i] = setup("/camp_object/campfire_pot_" + i, 2, 3);
	    }
		solidArea.x = gp.tileSize*9/10;
		solidArea.y = gp.tileSize*4/5;
		solidArea.width = gp.tileSize*6/5;
